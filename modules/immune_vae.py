...
class ProtVAE(keras.Model):
    ...
    def encoder_block(
        ...
        encoder_layer = layers.Dense(units=latent_dimension_size*8, activation="relu")(encoder_layer)
        encoder_layer = layers.Dense(
            units=latent_dimension_size * 4, activation="relu"
        )(encoder_layer)
    ...
    def decoder_block(
        ...
        concatenation_layer = layers.Dense(131, activation="relu")(latent_inputs)
        concatenation_layer = layers.Reshape((131, 1))(concatenation_layer)
        ...
        concatenation_layer = layers.Dense(527, activation="relu")(decoder_layer)
        concatenation_layer = layers.Reshape((527, 263))(concatenation_layer)
        decoder_layer = layers.Concatenate(axis=2)([upblock_layer, concatenation_layer])
        ...
        decoder_layer = layers.Conv1D(
            filters=32, kernel_size=2, strides=1, activation="elu"
        )(decoder_layer)
        # Final dense layer
    ...
    def train_step(self, data: npt.ArrayLike, cdr_constraint: bool = False) -> dict:
        """
        ## Training of the VAE
        ...
        See Bowman et al. 2015, Liu et al 2019. The difference is that we do not bound the control score with an upper and lower bound.
        """
        ## TO DO:
        ## THE OUTPUT FROM THE DECODER WILL ALWAYS BE ONE-HOT ENCODED
        ## IF WE CHOOSE ANY NON-ONE-HOT ENCODING, THEN THE RECONSTRUCTION
        ## WILL NEVER MATCH THE ENCODING. I.E. IF ANY NON-ONE-HOT ENCODING
        ## IS USED, A ONE-HOT-ENCODED VERSION NEEDS TO STILL EXIST TO CALCULATE
        ## THE LOSS WITH.

        ## ADD POSSIBILITY FOR CDR CONSTRAINT
        ...
        with tf.GradientTape() as tape:
            ...
            if self.pid:
                ...
                # self.current_control_score = control_score
...
if __name__ == "__main__":
    A = ProtVAE(input_shape=(130, 21), latent_dimension_size=65)
